...
class MusicRecommenderDataset(Dataset):
    ...
    def __init__(self, df: pd.DataFrame, mode: str = 'train', encoders=None):
        ...
        if encoders is not None:
            ...
            self.genres = self.genre_encoder.transform(df['main_genre'].values)
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        return {
            ...
            'playcount': torch.tensor(self.numerical_features[idx][11], dtype=torch.float)  # Target variable
        }
...
def main():
    ...
    encoders = torch.load('data/data_encoders.pt')
