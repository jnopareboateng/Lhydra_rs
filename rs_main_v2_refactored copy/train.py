...
def train(model, dataloader, criterion, optimizer, device):
    ...
    for batch in dataloader:
        ...
        print(f"ListNet Loss: {loss_details['listnet_loss']}, ILS Penalty: {loss_details['ils_penalty']}, Total Loss: {loss_details['total_loss']}")
...
def main():
    try:
        ...
        # Create dataloaders
        train_dataloader = create_dataloader(train_tensors, batch_size=TRAIN_BATCH_SIZE, shuffle=True)
        val_dataloader = create_dataloader(val_tensors, batch_size=VAL_BATCH_SIZE, shuffle=False)
        ...
        print(f"Release year classes: {len(preprocessor.release_year_encoder.categories_[0])}")
